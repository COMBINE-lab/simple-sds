...
fn mapped_vector<P, T>(filename: P, correct: &Vec<T>, name: &str)
where
    P: AsRef<Path>,
    T: Serializable + Debug + PartialEq,
{
    ...
    assert_eq!(
        mapped.is_empty(),
        correct.is_empty(),
        "Invaid emptiness for mapped slice of {}",
        name
    );
    assert_eq!(
        mapped.len(),
        correct.len(),
        "Invalid length for mapped slice of {}",
        name
    );
...
fn mapped_string<P: AsRef<Path>>(filename: P, correct: &String, name: &str) {
    ...
    assert_eq!(
        mapped.is_empty(),
        correct.is_empty(),
        "Invaid emptiness for mapped string slice of {}",
        name
    );
    assert_eq!(
        mapped.len(),
        correct.len(),
        "Invalid length for mapped string slice of {}",
        name
    );
    assert_eq!(
        *mapped, *correct,
        "Invalid mapped string slice for {}",
        name
    );
}
...
fn mapped_option<P: AsRef<Path>>(filename: P, correct: &Option<Vec<u64>>, name: &str) {
    ...
    if mapped.is_some() {
        assert_eq!(
            ...
            "Different results with unwrap() options for {}",
            name
        );
        assert_eq!(
            mapped.unwrap().as_ref(),
            correct.as_ref().unwrap().as_slice(),
            "Invalid content for {}",
            name
        );
    } else {
...
fn serialize_vec_u64_u64() {
    ...
    let filename = test(
        &original,
        "non-empty-vec-u64-u64",
        Some(1 + 2 * original.len()),
        false,
    )
    .unwrap();
...
fn serialize_bytes() {
    ...
    let filename = test(
        &padded,
        "padded-vec-u8",
        Some(1 + bits::bytes_to_words(padded.len())),
        false,
    )
    .unwrap();
    ...
    let filename = test(
        &unpadded,
        "unpadded-vec-u8",
        Some(1 + bits::bytes_to_words(unpadded.len())),
        false,
    )
    .unwrap();
...
fn serialize_option() {
    ...
    let filename = test(
        &some,
        "some-option",
        Some(1 + 1 + some.as_ref().unwrap().len()),
        false,
    )
    .unwrap();
...
fn skip_options() {
    ...
    assert_eq!(
        usize::load(&mut file).unwrap(),
        first,
        "Invalid value after skipped empty option"
    );
    skip_option(&mut file).unwrap();
    assert_eq!(
        usize::load(&mut file).unwrap(),
        second,
        "Invalid value after skipped non-empty option"
    );
    assert_eq!(
        Option::<usize>::load(&mut file).unwrap(),
        None,
        "Invalid absent option"
    );
    assert_eq!(
        usize::load(&mut file).unwrap(),
        third,
        "Invalid value after absent option"
    );
    drop(file);
