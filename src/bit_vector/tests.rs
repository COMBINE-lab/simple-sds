...
fn non_uniform_vector(regions: &[(usize, f64)], complement: bool) -> BitVector {
    ...
    assert_eq!(
        bv.count_ones(),
        total_ones,
        "Invalid number of ones in the non-uniform BitVector"
    );
...
fn non_empty_vector() {
    ...
    for (index, value) in bv.iter().enumerate() {
        assert_eq!(
            value,
            raw.bit(index),
            "Invalid value {} in the bitvector",
            index
        );
    }
...
fn from_iter() {
    ...
    assert_eq!(
        bv.len(),
        correct.len(),
        "Invalid length for a bitvector built from an iterator"
    );
    for i in 0..bv.len() {
        assert_eq!(
            bv.get(i),
...
fn empty_rank() {
    ...
    assert!(
        !empty.supports_rank(),
        "Rank support was enabled by default"
    );
    empty.enable_rank();
...
fn try_select(bv: &BitVector) {
    ...
    assert!(
        bv.select(bv.count_ones()).is_none(),
        "Got a result for select past the end"
    );
    assert!(
        bv.select_iter(bv.count_ones()).next().is_none(),
...
fn try_one_iter<T: Transformation>(bv: &BitVector) {
    ...
    while let Some((index, value)) = iter.next_back() {
        ...
        assert!(
            T::bit(bv, value),
            "OneIter returned an unset bit (backward)"
        );
...
fn empty_select() {
    let mut empty = BitVector::from(RawVector::new());
    assert!(
        !empty.supports_select(),
        "Select support was enabled by default"
    );
    empty.enable_select();
...
fn serialize_select() {
    ...
    bv.enable_select();
    assert!(
        bv.size_in_bytes() > old_size,
        "Select support did not increase the size in bytes"
    );
...
fn large_select() {
    ...
    assert_eq!(
        ss.long_superblocks(),
        3,
        "Invalid number of long superblocks"
    );
    assert_eq!(
        ss.short_superblocks(),
        4,
        "Invalid number of short superblocks"
    );
...
fn num_bits() {
    let mut bv = random_vector(1921, 0.5);
    let mut nb = std::mem::size_of::<usize>()
        + std::mem::size_of::<Option<RankSupport>>()
        + std::mem::size_of::<Option<SelectSupport<Identity>>>()
        + std::mem::size_of::<Option<SelectSupport<Complement>>>();
    nb = nb * 8 + bv.data.num_bits();
    assert_eq!(bv.num_bits(), nb);

    bv.enable_rank();
    let mut nb = std::mem::size_of::<usize>()
        + std::mem::size_of::<Option<SelectSupport<Identity>>>()
        + std::mem::size_of::<Option<SelectSupport<Complement>>>();
    nb = nb * 8 + bv.data.num_bits() + bv.rank.as_ref().unwrap().num_bits();
    assert_eq!(bv.num_bits(), nb);

    bv.enable_select();
    let mut nb =
        std::mem::size_of::<usize>() + std::mem::size_of::<Option<SelectSupport<Complement>>>();
    nb = nb * 8
        + bv.data.num_bits()
        + bv.rank.as_ref().unwrap().num_bits()
        + bv.select.as_ref().unwrap().num_bits();
    assert_eq!(bv.num_bits(), nb);
    ...
    nb = nb * 8
        ...
        + bv.rank.as_ref().unwrap().num_bits()
        + bv.select.as_ref().unwrap().num_bits()
        + bv.select_zero.as_ref().unwrap().num_bits();
    assert_eq!(bv.num_bits(), nb);
}
...
fn try_select_zero(bv: &BitVector) {
    assert!(
        bv.supports_select_zero(),
        "Failed to enable select zero support"
    );
    assert!(
        bv.select_zero(Complement::count_ones(bv)).is_none(),
        "Got a result for select past the end"
    );
    assert!(
        bv.select_zero_iter(Complement::count_ones(bv))
            .next()
            .is_none(),
        "Got a result for select_iter past the end"
    );
...
fn empty_select_zero() {
    let mut empty = BitVector::from(RawVector::new());
    assert!(
        !empty.supports_select_zero(),
        "Select zero support was enabled by default"
    );
    ...
    assert!(
        empty.select_zero(Complement::count_ones(&empty)).is_none(),
        "Got a result for select past the end"
    );
    assert!(
        empty
            .select_zero_iter(Complement::count_ones(&empty))
            .next()
            .is_none(),
        "Got a result for select_iter past the end"
    );
...
fn serialize_select_zero() {
    ...
    assert!(
        bv.size_in_bytes() > old_size,
        "Select zero support did not increase the size in bytes"
    );
...
fn empty_pred_succ() {
    let mut empty = BitVector::from(RawVector::new());
    assert!(
        !empty.supports_pred_succ(),
        "Predecessor/successor support was enabled by default"
    );
...
fn serialize_pred_succ() {
    ...
    assert!(
        bv.size_in_bytes() > old_size,
        "Predecessor/successor support did not increase the size in bytes"
    );
