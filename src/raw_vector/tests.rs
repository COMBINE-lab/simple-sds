...
#[test]
fn num_bits() {
    let v1 = RawVector::with_len(1, true);
    assert_eq!(std::mem::size_of::<RawVector>(), 32);
    let v1_bytes = std::mem::size_of::<usize>() + std::mem::size_of::<Vec<u64>>() + 8;
    let v1_nb = v1_bytes * 8;
    assert_eq!(v1.num_bits(), v1_nb);

    // needs 4x 512 bit u64 words
    let v2 = RawVector::with_len(2000, true);
    let v2_bytes = std::mem::size_of::<usize>() + std::mem::size_of::<Vec<u64>>() + 32 * 8;
...
fn initialization_vs_push() {
    ...
    assert_eq!(
        ones_set, ones_pushed,
        "Initializing with and pushing ones yield different vectors"
    );
    ...
    assert_eq!(
        zeros_set, zeros_pushed,
        "Initializing with and pushing zeros yield different vectors"
    );
}
...
fn push_bits() {
    ...
    assert_eq!(
        popped, source,
        "Invalid sequence of bits popped from RawVector"
    );
}
...
fn set_bits() {
    ...
    assert_eq!(
        v.count_ones(),
        0,
        "Non-zero bits in a zero-initialized vector"
    );
    assert_eq!(
        w.count_ones(),
        137,
        "Zero bits in an one-initialized vector"
    );
    for i in 0..137 {
        v.set_bit(i, i & 1 == 1);
        w.set_bit(i, i & 1 == 1);
    ...
    assert_eq!(
        v, w,
        "Fully overwritten vector still depends on the initialization value"
    );
...
fn set_ints() {
    ...
    for i in 0..64 {
        unsafe {
            v.set_int(bit_offset, i, 63);
            w.set_int(bit_offset, i, 63);
            bit_offset += 63;
            v.set_int(bit_offset, i * (i + 1), 64);
            w.set_int(bit_offset, i * (i + 1), 64);
            bit_offset += 64;
        }
    }
    assert_eq!(v.len(), 64 * (63 + 64), "Invalid vector length");
    ...
    for i in 0..64 {
        unsafe {
            assert_eq!(v.int(bit_offset, 63), i, "Invalid integer [{}].0", i);
            bit_offset += 63;
            assert_eq!(
                v.int(bit_offset, 64),
                i * (i + 1),
...
fn get_words() {
    ...
    unsafe {
        for (index, value) in correct.iter().enumerate() {
            assert_eq!(
                v.word_unchecked(index),
                *value,
                "Invalid integer {} (unchecked)",
                index
            );
        }
    }
...
fn push_bits_to_writer() {
    ...
    assert_eq!(
        v.len(),
        correct.len(),
        "Invalid size for the writer after push_bit"
    );
    v.close().unwrap();
...
fn push_ints_to_writer() {
    ...
    assert_eq!(
        w.len(),
        correct.len() * width,
        "Invalid size for the loaded vector"
    );
    for i in 0..correct.len() {
        unsafe {
...
fn check_mapper(mapper: &RawVectorMapper, truth: &RawVector, width: usize) {
    ...
    for i in 0..bits::bits_to_words(mapper.len()) {
        ...
        unsafe {
            assert_eq!(
                mapper.word_unchecked(i),
                truth.word_unchecked(i),
                "Invalid word {} (unchecked)",
                i
            );
        }
    }
